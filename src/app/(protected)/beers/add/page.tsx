              ...
              <div className="space-y-4">
                {results.map((beer) => {
                  ...
                  return (
                    <div
                      key={beer.id}
                      className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all hover:border-slate-300 hover:shadow-lg"
                    >
                      <div className="flex flex-col sm:flex-row">
                        {/* Clickable content area */}
                        <Link
                          href={`/beers/${beer.id}`}
                          className="flex flex-1 items-center gap-4 p-4 transition-colors hover:bg-slate-50"
                        >
                          {/* Larger beer image */}
                          <div className="relative h-24 w-24 min-h-24 min-w-24 shrink-0 overflow-hidden rounded-xl bg-slate-100">
                            <Image
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="mb-2 flex items-start justify-between gap-2">
                              ...
                              {beer.status === "pending" && (
                                <span className="shrink-0 rounded-full border border-yellow-300 bg-yellow-50 px-2 py-1 text-xs font-semibold text-yellow-700">
                                  Pending
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-sm text-slate-600">
                              ...
                              <span>{beer.countryName}</span>

                              {beer.volumeMl && (
                                <span>• {beer.volumeMl} ml</span>
                              )}
                        ...
                        {/* Action area */}
                        <div className="flex items-center justify-end border-t border-slate-100 p-4 sm:border-l sm:border-t-0">
                          <button
                            type="button"
                            onClick={() => void handleAddToCollection(beer.id)}
                            disabled={isAdded || isAdding}
                            className="w-full rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300 sm:w-auto"
                          >
                            {isAdding
                              ? "Adding..."
                              : isAdded
